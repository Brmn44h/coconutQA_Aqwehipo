...
class TestCinemaScope:
    ...
    def test_delete_movie_roles(self, user_role, expected_status, super_admin, admin, common_user, created_movie, movies_data):
        if user_role == "super_admin":
            super_admin.api.movies_api.delete_movie(created_movie, 200)
            super_admin.api.movies_api.get_movie(created_movie, 404)
        if user_role == "admin":
            admin.api.movies_api.delete_movie(created_movie, 403)
            admin.api.movies_api.get_movie(created_movie, 200)
        if user_role == "user":
            common_user.api.movies_api.delete_movie(created_movie, 403)
            common_user.api.movies_api.get_movie(created_movie, 200)
